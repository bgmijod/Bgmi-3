...
def configure_ngrok():
    ngrok_token = "2pybb9eDXacO4C0DPd6KbazCXmS_6XBEdGoF2pnVkdZRaYtSw"
    try:
...
def update_soul_txt(public_url):
    try:
        with open("jony3.txt", "w") as file:
            file.write(public_url)
        logging.info(f"New ngrok link saved in jony3.txt")
    except Exception as e:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jony3.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        logging.info("Updated jony3.txt on VPS successfully.")
    except Exception as e:
        logging.error(f"Failed to update jony3.txt on VPS: {str(e)}")
